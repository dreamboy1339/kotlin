...
object FirJsExportDeclarationChecker : FirBasicDeclarationChecker() {
    ...
    private val FirCallableDeclaration.isInlineWithReified: Boolean
        get() = when (this) {
            is FirPropertyAccessor -> {
                ...
                propertySymbol.fir.isInlineWithReified
            }
    ...
    private fun ConeKotlinType.isExportable(
        ...
        val isPrimitiveExportableType = with(session.typeContext) {
            val nonNullable = makeNonNullable()

            nonNullable.isAnyOrNullableAny() || nonNullable.isDynamic() ||
                    nonNullable is ConeKotlinType && isPrimitiveExportableConeKotlinType
        }
    ...
    private fun validateDeclarationOnConsumableName(
        ...
        if (!context.isTopLevel || declaration.name.isSpecial) {
            return
        }
        ...
        val name = (jsNameArgument as? FirConstExpression<*>)?.value as? String ?: declaration.name.asString()
